<?php
define('PODPRESS_VERSION', '8.8.10.18 beta 3');
/*
Info for WordPress:
==============================================================================
Plugin Name: podPress
Version: 8.8.10.18 beta 3
...
function podPress_class_init() {
	...
	/*  Load the activated podPress Modules (Add-Ons) */
	if ( TRUE === is_array($podPress->settings['podpress_modules']) AND FALSE === empty($podPress->settings['podpress_modules']) ) {
		foreach ($podPress->settings['podpress_modules'] as $module_slug => $module_data) {
			if ( TRUE === isset($module_data['active']) AND TRUE === $module_data['active'] ) {
				if ( 'podpress_wp_e-commerce_compatibility' == $module_slug ) {
					if ( ! defined( 'PODPRESS_MODULE_WP_ECOMMERCE_IS_ACTIVE' ) ) { define( 'PODPRESS_MODULE_WP_ECOMMERCE_IS_ACTIVE', TRUE ); }
				}
				require_once(PODPRESS_DIR.'/optional_files/'.$module_slug.'/'.$module_slug.'.php');
			}
		}
	}
	...
	if ( TRUE === defined( 'PODPRESS_WP_ECOMMERCE_IS_ACTIVE' ) AND TRUE == constant('PODPRESS_WP_ECOMMERCE_IS_ACTIVE') OR TRUE === defined( 'PODPRESS_MODULE_WP_ECOMMERCE_IS_ACTIVE' ) AND TRUE == constant('PODPRESS_MODULE_WP_ECOMMERCE_IS_ACTIVE') ) {
	...
	if ( TRUE === is_admin() ) {
		...
		if ( TRUE == version_compare($wp_version, '2.7', '>=') ) {
			...
			//  add the generalsettings script only this way in case WP  >= 2.8. Because this script needs to be add in the footer (after the admin_print_scripts hook)
			//~ if ( TRUE == version_compare($wp_version, '2.8', '>=') AND podPress_isset_upgrade_status('podpress_update_stats_table') OR podPress_isset_upgrade_status('podpress_update_statcounts_table') ) {
			
			add_action('admin_print_scripts-podpress/podpress_feed.php', 'podpress_print_admin_feedsettings_js');
			add_action('admin_print_styles-podpress/podpress_feed.php', 'podpress_print_admin_feedsettings_css');
			
			add_action('admin_print_scripts-podpress/podpress_general.php', 'podpress_print_admin_generalsettings_js');
			add_action('admin_print_styles-podpress/podpress_general.php', 'podpress_print_admin_generalsettings_css');
			
			add_action('admin_print_scripts-podpress/podpress_players.php', 'podpress_print_admin_playerssettings_js');
			add_action('admin_print_styles-podpress/podpress_players.php', 'podpress_print_admin_playerssettings_css');
			
			add_action('admin_print_scripts-widgets.php', 'podpress_print_admin_widgets_js');
			add_action('admin_print_styles-widgets.php', 'podpress_print_admin_widgets_css');
			
			add_action('admin_print_scripts-post.php', 'podpress_print_admin_js');
			add_action('admin_print_styles-post.php', 'podpress_print_admin_css');
			add_action('admin_print_scripts-post-new.php', 'podpress_print_admin_js');
			add_action('admin_print_styles-post-new.php', 'podpress_print_admin_css');
			add_action('admin_print_scripts-page.php', 'podpress_print_admin_js');
			add_action('admin_print_styles-page.php', 'podpress_print_admin_css');
			add_action('admin_print_scripts-page-new.php', 'podpress_print_admin_js');
			add_action('admin_print_styles-page-new.php', 'podpress_print_admin_css');
			add_action('admin_print_scripts-categories.php', 'podpress_print_admin_js');
			add_action('admin_print_styles-categories.php', 'podpress_print_admin_css');
			add_action('admin_print_scripts-admin.php', 'podpress_print_admin_js');
			add_action('admin_print_styles-admin.php', 'podpress_print_admin_css');
			add_action('admin_print_scripts-edit-tags.php', 'podpress_print_admin_js');
			add_action('admin_print_styles-edit-tags.php', 'podpress_print_admin_css');
			//~ }
		} else {
	...
	//}
	//~ add_filter('additional_plugin_locations', 'podpress_additional_plugin_location');
}

//~ function podpress_additional_plugin_location($list) {
	//~ $list[] = WP_PLUGIN_DIR.'/podpress/optional_files/modules';
	//~ return $list;
//~ }
...
// for WP 2.7+
function podpress_print_admin_feedsettings_js() {
	GLOBAL $wp_version;
	if ( TRUE == version_compare($wp_version, '2.8', '>=') ) { // for WP >= 2.8
...
// for WP 2.7+
function podpress_print_admin_feedsettings_css() {
	GLOBAL $wp_version;
	if ( TRUE == version_compare($wp_version, '2.8', '>=') ) { // for WP >= 2.8
		$plugins_url = plugins_url('', __FILE__);
	} else { // for WP 2.7.x (because the plugins_url() worked differently in WP 2.7.x)
	...
	wp_register_style( 'podpress_admin_styles', $plugins_url.'/style/podpress_admin_wp27plus.css');
	wp_enqueue_style( 'podpress_admin_styles' );
	wp_register_style( 'podpress_jquery_ui',  $plugins_url.'/js/jquery/css/custom-theme/jquery-ui-1.8.5.custom.css' );
...
// for WP 2.7+
function podpress_print_admin_generalsettings_js() {
	GLOBAL $wp_version;
	if ( TRUE == version_compare($wp_version, '2.8', '>=') ) { // for WP >= 2.8
		$plugins_url = plugins_url('', __FILE__);
	} else { // for WP 2.7.x (because the plugins_url() worked differently in WP 2.7.x)
	...
	wp_register_script( 'podpress_js', $plugins_url.'/js/podpress.js' );
	wp_register_script( 'podpress_admin_js', $plugins_url.'/js/podpress_admin.js' );
	wp_register_script( 'podpress_admin_gs_script',  $plugins_url.'/js/jquery/podpress_jquery_generalsettings.js', 'jquery', '1.1', TRUE );
	wp_enqueue_script( 'podpress_js' );
	wp_enqueue_script( 'podpress_admin_js' );
	...
	if ( podPress_isset_upgrade_status('podpress_update_stats_table') OR podPress_isset_upgrade_status('podpress_update_statcounts_table') ) {
		...
		wp_register_script( 'podpress_jquery_init',  $plugins_url.'/js/jquery/podpress_jquery_init.js' );
		wp_register_script( 'jquery-ui-accordion-dialog',  $plugins_url.'/js/jquery/jquery-ui-1.8.5.accordion_dialog.min.js' );
		wp_register_script( 'podpress_admin_gs_script', $plugins_url.'/js/jquery/podpress_jquery_ui_generalsettings_upgrade.js', 'jquery', '1.1', TRUE );
		wp_enqueue_script( 'podpress-jquery-ui-core' );
		wp_enqueue_script( 'podpress_jquery_init' );
		wp_enqueue_script( 'jquery-ui-accordion-dialog' );
		wp_enqueue_script( 'podpress_jquery_ui' );
...
// for WP 2.7+
function podpress_print_admin_generalsettings_css() {
	GLOBAL $wp_version;
	if ( TRUE == version_compare($wp_version, '2.8', '>=') ) { // for WP >= 2.8
		$plugins_url = plugins_url('', __FILE__);
	} else { // for WP 2.7.x (because the plugins_url() worked differently in WP 2.7.x)
...
// for WP 2.7+
function podpress_print_admin_playerssettings_css() {
	GLOBAL $wp_version;
	if ( TRUE == version_compare($wp_version, '2.8', '>=') ) { // for WP >= 2.8
		$plugins_url = plugins_url('', __FILE__);
	} else { // for WP 2.7.x (because the plugins_url() worked differently in WP 2.7.x)
...
// for WP 2.7+
function podpress_print_admin_widgets_js() {
	GLOBAL $wp_version;
	if ( TRUE == version_compare($wp_version, '2.8', '>=') ) { // for WP >= 2.8
		$plugins_url = plugins_url('', __FILE__);
	} else { // for WP 2.7.x (because the plugins_url() worked differently in WP 2.7.x)
...
// for WP 2.7+
function podpress_print_admin_widgets_css() {
	GLOBAL $wp_version;
	if ( TRUE == version_compare($wp_version, '2.8', '>=') ) { // for WP >= 2.8
		$plugins_url = plugins_url('', __FILE__);
	} else { // for WP 2.7.x (because the plugins_url() worked differently in WP 2.7.x)
...
function podPress_print_admin_js_and_css_old_wp() {
	...
	$page_with_podPress = Array('post.php', 'page.php', 'post-new.php', 'page-new.php', 'categories.php', 'admin.php', 'edit-tags.php', 'widgets.php');
	if ( in_array($pagenow, $page_with_podPress) ) {
...
function podPress_admin_footer() {
	GLOBAL $podPress, $pagenow, $wp_version;
	if(isset($podPress->settings['compatibilityChecks']['themeTested']) AND !$podPress->settings['compatibilityChecks']['themeTested']) {
		$podPress->settings['compatibilityChecks']['themeTested'] = true;
		podPress_update_option('podPress_config', $podPress->settings);
	}
	...
	if ( TRUE == version_compare($wp_version, '2.7', '>=') AND TRUE == version_compare($wp_version, '2.8', '<') ) {
		if ( 'admin.php' == $pagenow AND 'podpress/podpress_general.php' == $_GET['page'] AND (podPress_isset_upgrade_status('podpress_update_stats_table') OR podPress_isset_upgrade_status('podpress_update_statcounts_table')) ) {
			echo '<script type="text/javascript" src="'.plugins_url('podpress/js/jquery/podpress_jquery_ui_generalsettings_upgrade.js', __FILE__).'"></script>'."\n";
		}
	}
}
