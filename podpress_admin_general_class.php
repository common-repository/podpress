...
*/
	class podPressAdmin_class extends podPress_class
	{
		...
		function settings_general_edit() {
			...
			echo '				</td>'."\n";
			echo '				<td>'.__('This will enable the podPress statistics features.', 'podpress').'</td>'."\n";
			echo '			</tr> '."\n";
...
//			$duration_dividers = Array('colon' => '1:23:45:30', 'hminsms' => __('1 h 23 min 45 s 30 ms', 'podpress'), 'hrminsecmsec' => __('1 hr 23 min 45 sec 30 msec', 'podpress'), 'hoursminutessecondsmilliseconds' => __('1 hour 23 minutes 45 seconds 30 milliseconds', 'podpress'));
			...
			echo '		</table>'."\n";
			echo '	</fieldset>'."\n";
			$podpress_general_settingsurl = trailingslashit($adminurl).'admin.php?page='.plugin_basename(dirname(__FILE__).'/podpress_general.php');
			$podpress_modules = get_plugins('/podpress/optional_files/');
			echo '	<fieldset class="options">'."\n";
			echo '		<legend>'.__('Modules and Add-Ons', 'podpress').'</legend>'."\n";
			echo '		<table class="editform podpress_settings_table">'."\n";
			echo '			<tr>'."\n";
			echo '				<td>Modules in '.WP_PLUGIN_DIR.'/podpress/optional_files/'."\n";
			echo '				</td>'."\n";
			echo '			</tr> '."\n";
			echo '		</table>'."\n";
			//~ foreach ($podpress_modules as $key=>$metadata) {
				//~ $modules_mod['/podpress/optional_files/'.$key]=$metadata;
			//~ }
			//~ echo '	<pre>'."\n";
			//~ var_dump($podpress_general_settingsurl);
			//~ var_dump($podpress_modules);
			//~ echo '	</pre>'."\n";
			echo '		<input type="hidden" id="podpress_word_activate" value="'.__('Activate', 'podpress').'" />';
			echo '		<input type="hidden" id="podpress_word_deactivate" value="'.__('Deactivate', 'podpress').'" />';
			...
			echo '		<thead>'."\n";
			echo '		<tbody>'."\n";
			foreach ($podpress_modules as $key => $podpress_module) {
				$slug_tmp = preg_replace('/(.php)$/i', '', $key);
				$podpress_module['slug'] = preg_replace('/^(.+\/)/i', '', $slug_tmp);
				echo '			<tr>'."\n";
				//~ <!-- <span class="activate"><a class="edit" title="Activate this plugin" href="'.$podpress_general_settingsurl.'&action=activate_module&podpress_module='.attribute_escape($key).'&podPress_submitted=general">Activate</a></span> -->
				if ( isset($this->settings['podpress_modules'][$podpress_module['slug']]['active']) AND TRUE === $this->settings['podpress_modules'][$podpress_module['slug']]['active'] ) {
					echo '				<td><strong>'.$podpress_module['Name'].'</strong><div class="row-actions-visible"><input class="button-primary" type="button" name="Submit" id="'.$podpress_module['slug'].'_button" value="'.__('Deactivate').'" onclick="javascript:podpress_change_module_status(\''.$podpress_module['slug'].'\');" /><input type="hidden" id="'.$podpress_module['slug'].'_module_status" value="active" /></div></td>'."\n";
				} else {
				...
				if ( isset($podpress_module['AuthorURI']) AND FALSE === empty($podpress_module['AuthorURI']) ) {
					$module_author_str = '<a title="'.__('Visit author homepage').'" href="'.$podpress_module['AuthorURI'].'" target="_blank">'.$podpress_module['Author'].'</a>';
				} else {
					$module_author_str = $podpress_module['Author'];
				}
				echo '				<td><div class="plugin-description"><p>'.$podpress_module['Description'].'</p></div><div class="second plugin-version-author-uri">Version '.$podpress_module['Version'].' | By '.$module_author_str.'</div></td>'."\n";
				echo '			</tr> '."\n";
			}
			echo '		</tbody>'."\n";
			echo '		</table>'."\n";
		...
		function settings_general_save() {
			...
			if(isset($_GET['podpress_module'])) {
				$this->settings['podpress_module'][$_GET['podpress_module']] = $_GET['action'];
			} else {
				$this->settings['podpress_module'][$_GET['podpress_module']] = $_GET['action'];
			}
